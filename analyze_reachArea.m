...
% Analyzes normalized reach area data time-locked to when participants lifted their finger off the start position
...
%% Variables for normalized reach area and corresponding circle position
% Get normalized reach area for each trial
...
area_VertStat = nan(numSubs_total,2,96);
area_VertCW = nan(numSubs_total,2,96);
area_VertCCW = nan(numSubs_total,2,96);
area_HorzStat = nan(numSubs_total,2,96);
area_HorzCW = nan(numSubs_total,2,96);
area_HorzCCW = nan(numSubs_total,2,96);

for sub = 1:numSubs_total
        % Vertical stationary
        idx = vertOrHorzTargets(sub,:)==0 & rotCond(sub,:)==2 & correct(sub,:)==1;
        area_VertStat(sub,1,1:sum(idx)) = reachAreaNorm(sub,idx);
        area_VertStat(sub,2,1:sum(idx)) = endPos_collapsed(sub,idx);

        % Vertical clockwise
        idx = vertOrHorzTargets(sub,:)==0 & rotCond(sub,:)==0 & correct(sub,:)==1;
        area_VertCW(sub,1,1:sum(idx)) = reachAreaNorm(sub,idx);
        area_VertCW(sub,2,1:sum(idx)) = endPos_collapsed(sub,idx);

        % Vertical counterclockwise
        idx = vertOrHorzTargets(sub,:)==0 & rotCond(sub,:)==1 & correct(sub,:)==1;
        area_VertCCW(sub,1,1:sum(idx)) = reachAreaNorm(sub,idx);
        area_VertCCW(sub,2,1:sum(idx)) = endPos_collapsed(sub,idx);

        % Horizontal stationary
        idx = vertOrHorzTargets(sub,:)==1 & rotCond(sub,:)==2 & correct(sub,:)==1;
        area_HorzStat(sub,1,1:sum(idx)) = reachAreaNorm(sub,idx);
        area_HorzStat(sub,2,1:sum(idx)) = endPos_collapsed(sub,idx);

        % Horizontal clockwise
        idx = vertOrHorzTargets(sub,:)==1 & rotCond(sub,:)==0 & correct(sub,:)==1;
        area_HorzCW(sub,1,1:sum(idx)) = reachAreaNorm(sub,idx);
        area_HorzCW(sub,2,1:sum(idx)) = endPos_collapsed(sub,idx);

        % Horizontal counterclockwise
        idx = vertOrHorzTargets(sub,:)==1 & rotCond(sub,:)==1 & correct(sub,:)==1;
        area_HorzCCW(sub,1,1:sum(idx)) = reachAreaNorm(sub,idx);
        area_HorzCCW(sub,2,1:sum(idx)) = endPos_collapsed(sub,idx);
end
...
opts.Display = 'off';
LB = [-2 0.01 -inf]; % Lower bound for each parameter
UB = [2 2 inf]; % Upper bound for each parameter
...
for sub = 1:numSubs_total
    ...
    if cond==1
        data = squeeze(area_VertStat(sub,:,:));
    elseif cond==2
        data = squeeze(area_VertCW(sub,:,:));
    elseif cond==3
        data = squeeze(area_VertCCW(sub,:,:));
    elseif cond==4
        data = squeeze(area_HorzStat(sub,:,:));
    elseif cond==5
        data = squeeze(area_HorzCW(sub,:,:));
    elseif cond==6
        data = squeeze(area_HorzCCW(sub,:,:));
    end
...
for sub = 1:numSubs_total
    % Horizontal Stationary
    SSModel = sine61Cost(params_HorzStat(sub,:),squeeze(area_HorzStat(sub,:,:)));
    SSTotal = nansum((squeeze(area_HorzStat(sub,1,:)) - nanmean(squeeze(area_HorzStat(sub,1,:)))).^2);
    r2_values(sub,1) = 1-(SSModel/SSTotal);
    
    % Horizontal Clockwise
    SSModel = sine61Cost(params_HorzCW(sub,:),squeeze(area_HorzCW(sub,:,:)));
    SSTotal = nansum((squeeze(area_HorzCW(sub,1,:)) - nanmean(squeeze(area_HorzCW(sub,1,:)))).^2);
    r2_values(sub,2) = 1-(SSModel/SSTotal);
    
    % Horizontal Counterclockwise
    SSModel = sine61Cost(params_HorzCCW(sub,:),squeeze(area_HorzCCW(sub,:,:)));
    SSTotal = nansum((squeeze(area_HorzCCW(sub,1,:)) - nanmean(squeeze(area_HorzCCW(sub,1,:)))).^2);
    r2_values(sub,3) = 1-(SSModel/SSTotal);
    
    % Vertical Stationary
    SSModel = sine61Cost(params_VertStat(sub,:),squeeze(area_VertStat(sub,:,:)));
    SSTotal = nansum((squeeze(area_VertStat(sub,1,:)) - nanmean(squeeze(area_VertStat(sub,1,:)))).^2);
    r2_values(sub,4) = 1-(SSModel/SSTotal);
    
    % Vertical Clockwise
    SSModel = sine61Cost(params_VertCW(sub,:),squeeze(area_VertCW(sub,:,:)));
    SSTotal = nansum((squeeze(area_VertCW(sub,1,:)) - nanmean(squeeze(area_VertCW(sub,1,:)))).^2);
    r2_values(sub,5) = 1-(SSModel/SSTotal);
    
    % Vertical Counterclockwise
    SSModel = sine61Cost(params_VertCCW(sub,:),squeeze(area_VertCCW(sub,:,:)));
    SSTotal = nansum((squeeze(area_VertCCW(sub,1,:)) - nanmean(squeeze(area_VertCCW(sub,1,:)))).^2);
    r2_values(sub,6) = 1-(SSModel/SSTotal);
end


%% Save R2 values to csv (if desired)
csvwrite('R2_reachArea.csv',r2_values);
...
saveas(gcf,'parameters_reachArea.pdf'); % As .pdf for vector graphics


%% Make a figure to demonstrate single-trial sine wave fitting
% For example, let's plot Subject 11's normalized reach area data from all
% their Horizontal Clockwise trials

sub = 11; % Pick a subject to plot
scatterSize = 70; % Size of scatter plot points

% Make a new figure
figure; hold on;
scatter(squeeze(area_HorzCW(sub,2,:)),squeeze(area_HorzCW(sub,1,:)),scatterSize,'b','filled','MarkerEdgeColor','k');
plot(1:61,sine61Out(params_HorzCW(sub,:)),'Color','k','LineWidth',1);
% Plotting options
ylim([-2.25 3.25]); xlim([0 62]);
title('Subject 11 Horizontal CW Box');
xticks([1 31 61]); xticklabels({'100%' '50%' '100%'});
yticks([-2 0 2]); yticklabels({'-2' '0' '2'});
...
%% Save figure (if desired)
saveas(gcf,'singleTrialFit_reachArea.pdf'); % As .pdf for vector graphics
...
% Add last column indicating which experiment participant belonged to (Box=1; Arrow=2)
area_Amplitude(:,end+1) = experiment;
...
% Add last column indicating which experiment participant belonged to (Box=1; Arrow=2)
area_Phase(:,end+1) = experiment;


%% Save .csv files of data for statistics elsewhere (if desired)
csvwrite('area_Mean.csv',area_Mean);
csvwrite('area_Amplitude.csv',area_Amplitude);
csvwrite('area_Phase.csv',area_Phase);
...
for cond = 1:3
    ...
    tmpAngles = circ_mean(area_Phase(:,[cond cond+3])');
...
for cond = 1:3
    ...
    % Get phase estimates in this condition
    tmpAngles1 = wrapTo2Pi(circ_mean([area_Phase(:,cond) area_Phase(:,cond+3)]'));
    % Get phase estimates in another condition
    tmpAngles2 = wrapTo2Pi(circ_mean([area_Phase(:,cond2) area_Phase(:,cond2+3)]'));
...
saveas(gcf,'reachArea_Phase.pdf'); % As .pdf for vector graphics
...
%% Save figure
saveas(gcf,'reachArea.pdf'); % As .pdf for vector graphics
